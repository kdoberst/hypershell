...
# Fail closed on schema drift: a missing or non-list "issues" key must not
# be treated as a clean pass (e.g. {} or a renamed field).
if not isinstance(data, dict):
    scan_incomplete(
        f"WARNING: SkillSpector scan output at {scan_out} is not a JSON object"
    )
if "issues" not in data:
    scan_incomplete(
        f"WARNING: SkillSpector scan output at {scan_out} is missing required key 'issues'"
    )
issues = data["issues"]
if not isinstance(issues, list):
    scan_incomplete(
        f"WARNING: SkillSpector scan output at {scan_out} has non-list 'issues'"
    )
...
risk_severe = risk_sev in ("HIGH", "CRITICAL")

if not severe and not risk_severe:
...
print("=========================================")
if risk_severe and not severe:
    print(
        f"  {risk_sev}: risk_assessment.severity with no HIGH/CRITICAL entries in issues"
    )
for issue in severe:
