...
**Spec corpus**: 47 spec files; the coverage table tracks 38 analyzed feature/spec groups
**Codebase commit**: working tree (cluster memory + cluster CPU + cluster pods + cluster nodes Prometheus/kube-state-metrics scrape, BFF routes, dashboard adapters + registered users API); e2e performance + OpenShift driver
