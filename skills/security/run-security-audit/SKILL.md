...
# Fetch ONLY the pinned ref - never clone the default branch tip first.
...
# Also clean up on EXIT in case a later command aborts the shell, but do not
# rely on the trap alone - agent turns often use separate shells.
...
if [ ! -d "$HARNESS_DIR/.git" ]; then
  ...
  git -C "$HARNESS_DIR" remote add origin "$HARNESS_URL"
else
  git -C "$HARNESS_DIR" remote set-url origin "$HARNESS_URL"
fi

git -C "$HARNESS_DIR" fetch --depth 1 origin "$HARNESS_REF"
git -C "$HARNESS_DIR" checkout --force FETCH_HEAD

actual="$(git -C "$HARNESS_DIR" rev-parse HEAD)"
# When HARNESS_REF is a full commit SHA, require an exact match. Tag/branch
# overrides are allowed via AI_SECURITY_HARNESS_REF and resolve through FETCH_HEAD.
if [[ "$HARNESS_REF" =~ ^[0-9a-f]{40}$ ]] && [[ "$actual" != "$HARNESS_REF" ]]; then
  echo "STOP: harness HEAD $actual does not match pinned ref $HARNESS_REF" >&2
  exit 1
fi
echo "Harness pinned at $actual (requested $HARNESS_REF)"

# Clear credentials from this shell before any later non-GitLab git work.
_harness_git_cleanup
trap - EXIT
```

If the fetch/checkout fails (network, auth), stop and tell the user - the harness
...
GitLab instance hostname. Do not proceed to pip install until the pin check above
has succeeded.
...
- The `contracts/` submodule in the harness may not be initialized in a shallow clone. If `validate_report.py` fails because the schema is missing, re-create the Step 2 `GIT_ASKPASS` helper for that command only, run `git -C $HARNESS_DIR submodule update --init contracts`, then clear `GIT_ASKPASS` again immediately. Do not leave askpass exported across later steps.
