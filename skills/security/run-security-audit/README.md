...
1. **Bootstraps the harness** - fetches only the pinned commit of
   `ai-security-harness` into
   `apm_modules/hybrid-platforms-sec/ai-security-harness/` (already
   gitignored), verifies `HEAD` matches the pin when the pin is a full
   SHA, clears `GIT_ASKPASS`, then installs Python dependencies.
