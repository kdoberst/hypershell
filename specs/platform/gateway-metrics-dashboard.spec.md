...
`GatewayMetricsDashboard` remains the canonical component for Prometheus-sourced phase counts. Hosts MAY embed it on any route; the `/dashboard` route is owned by the operational dashboard spec and renders `OperationalDashboardPage` instead.
...
The Prometheus Operator SHALL be vendored as `deploy/kind/prometheus-operator/prometheus-operator-bundle.yaml` (pinned to a specific release) and applied as a cluster-level infrastructure dependency before application manifests, in the same layer as cert-manager and CNPG.
...
The BFF SHALL recognise `/dashboard` as a valid SPA shell route (returning `index.html` for direct navigation and refresh) alongside `/`, `/login`, `/gateways/new`, and `/gateways/:gatewayId`.
...
Which React component renders at `/dashboard` is defined by `web-console/operational-dashboard.spec.md` (currently `OperationalDashboardPage`). This spec does not require `GatewayMetricsDashboard` on that route.
